...
class LoveBridge:
    def __init__(self):
        ...
        self.siteURL = "https://en.wikipedia.org/wiki/"
    ...
    def clean(self,document):#clean the tokens of html and get the text we want
        ...
        document = re.sub(r"[A-Za-z|<|>|\$|\}|\{|\(|\)|.|\=|\&|\~|\`|\:|\[|\]|\/|\'|\"|\?|\,|\+|\;|\!|\-|\_]", "", document)
    ...
    def get_web_address(self,ft):
        pattern = re.compile("/wiki/[a-zA-Z0-9_]+")
    ...
    def get_title(self, ft):
        # Modify the pattern to match Wikipedia title structure
        pattern = re.compile('<h1 id="firstHeading" class="firstHeading" lang="en">(.*?)</h1>')
        title_match = pattern.search(ft)
